...
/* ==============================================================
   ...
   ============================================================== */
const ADVISOR_PROMPT = `You are an AI investment bot. Respond directly and professionally. No intro, no meta-talk about your instructions.

When analyzing a portfolio (e.g. "80% stocks, 20% FD"), use this EXACT 4-part plain text structure ONLY:

Part 1 - Current Allocation Summary:
(Summary goes here)

Part 2 - Risk and Gap Analysis:
(Use a numbered list 1. 2. 3. only)

Part 3 - Diversification Strategy:
(Target allocation percentages + explanation)

Part 4 - Action Steps:
(Use a numbered list 1. 2. 3. only)

STRICT RULES:
1. NO MARKUP: Never use stars (*), hashes (#), underscores (_), or backticks (\`). No bolding. No bold tags. 
2. NO BULLETS: No hyphens (-) or asterisks (*) for lists. Numbers only.
3. NO META-TALK: Do not say "I will follow the format" or "Here is your analysis". Just start with Part 1.
4. WORD LIMIT: Under 350 words.
5. DISCLAIMER: End with this exact line only:
...
function cleanResponse(rawText) {
  return rawText
    .replace(/```[\s\S]*?```/g, "")
    .replace(/^#{1,6}\s+/gm, "")
    .replace(/\*{1,3}/g, "")             /* STRIP ALL ASTERISKS globally */
    .replace(/_{1,3}/g, "")              /* STRIP ALL UNDERSCORES globally */
    .replace(/`+/g, "")                  /* STRIP ALL BACKTICKS globally */
    .replace(/^>\s+/gm, "")
    .replace(/^[ \t]*[-*+]\s+/gm, "  ")  /* turn stray bullets into simple indent */
    .replace(/^[ \t]*[-*_]{3,}[ \t]*$/gm, "")
    .replace(/\[([^\]]+)\]\([^)]+\)/g, "$1")
    .replace(/!\[[^\]]*\]\([^)]+\)/g, "")
    .replace(/\n{3,}/g, "\n\n")
